<template>
    <div>
        <header>
            <h2>
                <router-link
                        :to="{ name: 'dashboard' }"
                >
                    Tableau de bord
                </router-link>
                /
                <router-link
                        :to="{ name: 'actionTrip' }"
                >
                    Voyage
                </router-link>
                /
                <span class="current__breadcrump">Proposer un voyage</span>
            </h2>
        </header>
        <div class="separation"></div>
        <div class="content_main">
            <form-wizard
                    color="rgba(var(--vs-primary), 1)"
                    :title="null"
                    :subtitle="null"
                    finishButtonText="Envoyer"
                    @on-complete="addTrip"
                    back-button-text="Précédent"
                    next-button-text="Suivant"
            >
                    <tab-content title="Informations" class="mb-5 profile-card" icon="fas fa-info" :before-change="validateStep1">
                        <!-- tab 1 content -->
                        <vs-row>
                            <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="7">
                                <vs-row>
                                    <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="6">
                                        <vs-input
                                                label-placeholder="Petit nom pour votre voyage"
                                                v-model="trip.title"
                                                class="input__add__trip"
                                        >
                                            <template v-if="errors.includes('title')" #message-danger>
                                                Un titre pour votre voyage est requis
                                            </template>
                                        </vs-input>
                                    </vs-col>
                                    <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="6">
                                        <vs-input
                                                type="number"
                                                v-model="trip.nbTravelers"
                                                label="Nombre de voyageurs recherchés"
                                                min="1"
                                                max="11"
                                                class="input__add__trip"
                                        />
                                    </vs-col>
                                    <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="12">
                                        <label for="description__trip" class="label__textaera">Courte présentation de votre voyage</label>
                                        <textarea
                                            id="description__trip"
                                            v-model="trip.description"
                                            class="textaera__add__trip"
                                            rows="16"
                                        >
                                        </textarea>
                                        <div v-if="errors.includes('description')" class="msg__error">
                                            Une courte description est requise
                                        </div>
                                    </vs-col>
                                </vs-row>
                            </vs-col>
                            <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="5">
                                <div class="step-form-wizard">
                                    <svg id="bebdbc02-5f56-4bd5-9ad9-bce5f07a2c3e" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="933.44272" height="783.89357" viewBox="0 0 933.44272 783.89357"><path d="M627.33188,841.14167c0,15.28692-94.57823,27.67192-211.44706,27.7519h-1.06254c-57.04625,0-108.84836-2.93629-147.02008-7.7006-1.6338-.21709-3.2448-.42273-4.82149-.63981-6.48953-.8569-12.55633-1.78234-18.1433-2.74209-26.70079-4.65009-42.52473-10.41982-42.52473-16.6694,0-5.06144,10.35127-9.79144,28.43745-13.8817,9.55149-2.15939,21.25094-4.1245,34.73271-5.86116q3.719-.497,7.63206-.93688c37.60053-4.39869,87.26605-7.07223,141.70738-7.07223,10.57979,0,20.98817.10286,31.15667.29707C548.56622,815.65188,627.33188,827.19142,627.33188,841.14167Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><rect x="617.70498" y="416.06562" width="314.75413" height="161.31149" fill="#f2f2f2"/><rect x="226.22953" y="0.98361" width="314.75413" height="161.31149" fill="#f2f2f2"/><path d="M615.74591,248.27871H299.02457V85H615.74591Zm-314.75413-1.96722H613.7787V86.96721H300.99178Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><rect x="617.70498" y="0.98361" width="314.75413" height="161.31149" fill="#f2f2f2"/><rect x="226.22953" y="208.52461" width="314.75413" height="161.31149" fill="#f2f2f2"/><rect x="617.70498" y="208.52461" width="314.75413" height="161.31149" fill="#f2f2f2"/><rect x="226.22953" y="416.06562" width="314.75413" height="161.31149" fill="#f2f2f2"/><circle cx="40.32787" cy="81.63935" r="40.32787" fill="#f2f2f2"/><path d="M227.22128,126.31148a40.32788,40.32788,0,1,0,40.32787,40.32787A40.32787,40.32787,0,0,0,227.22128,126.31148Zm17.47631,55.31058-3.47721,3.47721-14.4909-14.49091-14.49091,14.49091-3.47721-3.47721,14.49091-14.4909L206.79415,150.673l3.4772-3.4772L226.72948,163.654l16.45812-16.45812,3.4772,3.4772-16.45812,16.45813Z" transform="translate(-73.77864 -85)" fill="#f2f2f2"/><polygon points="36.256 100.783 17.71 76.237 21.634 73.271 36.531 92.988 78.311 45.105 82.017 48.338 36.256 100.783" v-bind:fill="this.user.color"/><path d="M580.49352,662.04422q-19.18523-3.25031-36.90106-5.58273l4.669-9.05252c-1.57439-.54682-8.49144,4.63729-8.49144,4.63729l6.1273-26.27728c-7.9182.9549-11.9441,27.78561-11.9441,27.78561l-8.84669-9.07128,4.283,10.1985a816.82143,816.82143,0,0,0-93.62328-5.89217l3.99619-7.748c-1.57439-.54682-8.49144,4.63729-8.49144,4.63729l6.12729-26.27729c-7.91819.95491-11.9441,27.78562-11.9441,27.78562l-8.84668-9.07129,4.53634,10.80189a525.9061,525.9061,0,0,0-53.82008,3.65579c4.042-12.07,17.7728-23.54953,17.7728-23.54953-10.48615,3.11875-15.98671,8.34487-18.86091,13.21713a211.63887,211.63887,0,0,1,12.23271-73.124s-21.67451,47.16568-18.90888,78.82526l.33079,5.62183c-18.3751,2.63107-25.60407,8.81279-25.60407,8.81279Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><circle cx="355.70497" cy="255.58758" r="34.78687" fill="#ff6584"/><polygon points="728.996 475.767 724.918 472.504 747.757 452.112 805.67 452.112 805.67 470.057 768.149 495.343 728.996 475.767" fill="#3f3d56"/><polygon points="816.229 463.551 816.229 452.52 808.54 452.52 808.54 455.221 805.67 452.112 786.909 472.504 779.568 477.398 768.186 560.56 785.352 577.726 825.246 577.726 825.246 473.32 816.229 463.551" fill="#ccc"/><rect x="724.91811" y="472.50395" width="61.99121" height="105.22193" fill="#f2f2f2"/><rect x="748.98049" y="552.03216" width="11.41943" height="25.69373" fill="#ccc"/><polygon points="736.745 559.373 736.745 555.295 731.851 555.295 731.851 559.373 736.338 559.373 736.745 559.373" fill="#3f3d56"/><polygon points="736.338 562.636 731.851 562.636 731.851 566.714 736.745 566.714 736.745 562.636 736.338 562.636" fill="#3f3d56"/><rect x="740.00807" y="555.29485" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="740.00807" y="562.63592" width="4.89404" height="4.07837" fill="#3f3d56"/><polygon points="770.188 559.373 770.188 555.295 765.294 555.295 765.294 559.373 769.78 559.373 770.188 559.373" fill="#3f3d56"/><polygon points="769.78 562.636 765.294 562.636 765.294 566.714 770.188 566.714 770.188 562.636 769.78 562.636" fill="#3f3d56"/><rect x="773.4507" y="555.29485" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="773.4507" y="562.63592" width="4.89404" height="4.07837" fill="#3f3d56"/><polygon points="804.446 477.806 804.446 473.727 799.552 473.727 799.552 477.806 804.038 477.806 804.446 477.806" fill="#3f3d56"/><polygon points="804.038 481.069 799.552 481.069 799.552 485.147 804.446 485.147 804.446 481.069 804.038 481.069" fill="#3f3d56"/><rect x="807.709" y="473.72747" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="807.709" y="481.06853" width="4.89404" height="4.07837" fill="#3f3d56"/><polygon points="804.446 500.645 804.446 496.566 799.552 496.566 799.552 500.645 804.038 500.645 804.446 500.645" fill="#3f3d56"/><polygon points="804.038 503.907 799.552 503.907 799.552 507.986 804.446 507.986 804.446 503.907 804.038 503.907" fill="#3f3d56"/><rect x="807.709" y="496.56633" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="807.709" y="503.9074" width="4.89404" height="4.07837" fill="#3f3d56"/><polygon points="804.446 523.484 804.446 519.405 799.552 519.405 799.552 523.484 804.038 523.484 804.446 523.484" fill="#3f3d56"/><polygon points="804.038 526.746 799.552 526.746 799.552 530.825 804.446 530.825 804.446 526.746 804.038 526.746" fill="#3f3d56"/><rect x="807.709" y="519.4052" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="807.709" y="526.74627" width="4.89404" height="4.07837" fill="#3f3d56"/><polygon points="804.446 546.322 804.446 542.244 799.552 542.244 799.552 546.322 804.038 546.322 804.446 546.322" fill="#3f3d56"/><polygon points="804.038 549.585 799.552 549.585 799.552 553.664 804.446 553.664 804.446 549.585 804.038 549.585" fill="#3f3d56"/><rect x="807.709" y="542.24407" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="807.709" y="549.58514" width="4.89404" height="4.07837" fill="#3f3d56"/><rect x="731.85133" y="477.80583" width="47.30908" height="9.78809" fill="#3f3d56"/><rect x="731.85133" y="497.38201" width="47.30908" height="9.78809" fill="#3f3d56"/><rect x="731.85133" y="516.95818" width="47.30908" height="9.78809" fill="#3f3d56"/><rect x="731.85133" y="536.53435" width="47.30908" height="9.78809" fill="#3f3d56"/><polygon points="355.902 469.793 397.421 469.793 397.421 459.771 420.328 459.771 420.328 469.793 463.279 469.793 463.279 514.176 420.328 514.176 420.328 570.012 397.421 570.012 397.421 514.176 355.902 514.176 334.426 489.837 355.902 469.793" fill="#3f3d56"/><path d="M551.81148,454.8361h-1.96722a96.39345,96.39345,0,1,0-192.7869,0h-1.96721a98.36067,98.36067,0,0,1,196.72133,0Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><rect x="404.17666" y="355.98722" width="2.51384" height="99.29687" transform="matrix(1, -0.00188, 0.00188, 1, -74.53899, -84.23861)" fill="#3f3d56"/><path d="M429.821,368.59564c.22517,34.72128-24.15028,63.0277-24.15028,63.0277s-24.74052-27.98789-24.96569-62.70917,24.15027-63.0277,24.15027-63.0277S429.59582,333.87436,429.821,368.59564Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><path d="M429.821,368.59564c.22517,34.72128-24.15028,63.0277-24.15028,63.0277s-24.74052-27.98789-24.96569-62.70917,24.15027-63.0277,24.15027-63.0277S429.59582,333.87436,429.821,368.59564Z" transform="translate(-73.77864 -85)" opacity="0.1"/><rect x="371.49674" y="356.04854" width="2.51384" height="99.29687" transform="matrix(1, -0.00188, 0.00188, 1, -74.53916, -84.29992)" fill="#3f3d56"/><path d="M397.14106,368.657c.22518,34.72128-24.15027,63.0277-24.15027,63.0277s-24.74052-27.9879-24.96569-62.70918,24.15027-63.0277,24.15027-63.0277S396.91589,333.93568,397.14106,368.657Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><rect x="465.54425" y="291.88024" width="1.99013" height="78.61032" fill="#3f3d56"/><path d="M559.67971,386.89823c.12669,27.48807-19.21262,49.86116-19.21262,49.86116s-19.54472-22.19388-19.67141-49.68195,19.21263-49.86116,19.21263-49.86116S559.553,359.41015,559.67971,386.89823Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><circle cx="770.60875" cy="291.63937" r="68.40773" fill="#3f3d56"/><polygon points="777.45 266.171 777.45 294.807 735.823 294.807 728.132 304.034 729.01 294.807 725.269 294.807 725.269 266.171 777.45 266.171" v-bind:fill="this.user.color"/><rect x="758.67717" y="276.03476" width="18.77235" height="18.77235" opacity="0.1"/><polygon points="762.813 280.171 814.994 280.171 814.994 308.807 811.252 308.807 812.131 318.034 804.441 308.807 762.813 308.807 762.813 280.171" v-bind:fill="this.user.color"/><ellipse cx="728.4505" cy="307.53414" rx="11.45432" ry="1.11749" opacity="0.1"/><ellipse cx="812.13065" cy="322.48838" rx="13.04519" ry="1.2727" opacity="0.1"/><path d="M883.2215,106.0316H838.018a9.60936,9.60936,0,0,0-9.37409,7.496L805.80347,214.83763a7.76645,7.76645,0,0,0,7.1941,9.46513l46.46951,2.28955a10.2603,10.2603,0,0,0,10.60137-8.42156l12.95649-71.62734.9639.2754,2.47911-13.57606-.99221-.24805,3.61661-19.99376A5.91643,5.91643,0,0,0,883.2215,106.0316Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><path d="M843.969,108.21557h-5.27661a6.64081,6.64081,0,0,0-6.48818,5.22521l-21.92367,100.4835a4.11488,4.11488,0,0,0,3.75119,4.98323l46.31918,3.03571a5.32916,5.32916,0,0,0,5.59329-4.37305l18.70761-103.85948a4.66768,4.66768,0,0,0-4.59375-5.49512h-5.00569a2.20288,2.20288,0,0,0-2.12714,1.6302h0a1.94476,1.94476,0,0,1-1.87789,1.43917h-23.7638a3.32434,3.32434,0,0,1-3.31454-3.06937Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><circle cx="389.94677" cy="81.78849" r="65.46092" v-bind:fill="this.user.color"/><path d="M520.72517,207.888c-12.56848.00078-40.32306-12.90833-65.00687-28.02709-30.63032-18.761-60.3785-43.28289-55.17787-51.77425,1.47-2.39963,5.11342-3.02717,11.14053-1.92079l-.43212,2.35291c-4.66729-.85879-7.82566-.559-8.66849.81675-1.2294,2.00722,1.48128,7.83423,12.77519,18.16852,10.29225,9.41867,25.07038,20.18508,41.61241,30.31694,16.54164,10.13186,32.84776,18.40443,45.91376,23.294,14.33783,5.36608,20.76007,5.13172,21.9887,3.12606.844-1.37811-.32857-4.33132-3.21755-8.10362l1.899-1.45441c3.73063,4.87167,4.82923,8.40649,3.35847,10.8069C525.90049,207.139,523.75351,207.888,520.72517,207.888Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><circle cx="381.86671" cy="48.14092" r="13.38973" fill="#fff"/><circle cx="385.74494" cy="121.54293" r="8.92649" fill="#fff"/><circle cx="416.48373" cy="90.94044" r="8.92649" fill="#fff"/><path d="M1007.22136,663.36071H690.5V500.082h316.72134ZM692.46723,661.3935h312.78692V502.04922H692.46723Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><path d="M1005.25415,456.80332H688.5328V293.52461h316.72135ZM690.5,454.8361h312.78691V295.49182H690.5Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><path d="M298.18879,805.30317c5.36982,19.85008,23.76267,32.14374,23.76267,32.14374s9.68665-19.88969,4.31683-39.73977-23.76267-32.14374-23.76267-32.14374S292.819,785.45308,298.18879,805.30317Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><path d="M306.07936,801.03717c14.73487,14.3438,16.53462,36.39356,16.53462,36.39356s-22.09018-1.20614-36.825-15.54994-16.53463-36.39355-16.53463-36.39355S291.34449,786.69337,306.07936,801.03717Z" transform="translate(-73.77864 -85)" v-bind:fill="this.user.color"/><path d="M357.556,528.63387l-15.70335,1.74482s-6.97927,31.4067,0,73.28231S357.556,687.41221,357.556,687.41221s6.97927,136.09571,10.4689,137.84053,34.89634,12.21372,36.64115,1.74482,1.74482-249.50882,1.74482-249.50882l24.42744,115.15792s3.48963,144.8198,10.4689,144.8198,29.66188-1.74482,29.66188-13.95854S476.20354,650.771,476.20354,650.771s-1.74481-118.64754-8.72408-122.13718S357.556,528.63387,357.556,528.63387Z" transform="translate(-73.77864 -85)" fill="#2f2e41"/><path d="M371.51453,820.01829s-15.70335-1.74482-19.193,0-8.72409,13.95853,0,20.9378,31.4067,17.44817,38.386,17.44817,13.95853-3.48963,13.95853-3.48963l-3.48963-29.66189Z" transform="translate(-73.77864 -85)" fill="#2f2e41"/><path d="M441.30721,828.74237l1.74481,24.42744s22.68262,3.48963,24.42744,0,19.193-12.21372,20.9378-13.95854,3.48964-20.9378-6.97926-22.68262-13.95854,3.48964-13.95854,3.48964Z" transform="translate(-73.77864 -85)" fill="#2f2e41"/><path d="M481.85415,321.01532s38.147-24.53087,37.26414-41.38406-23.87951-69.9918-23.87951-69.9918-17.99016-37.4032-25.14209-22.11729,9.39989,31.55733,9.39989,31.55733l10.6833,52.12683-29.03429,27.69849Z" transform="translate(-73.77864 -85)" fill="#a0616a"/><circle cx="313.43924" cy="169.69762" r="31.4067" fill="#a0616a"/><path d="M371.51453,273.89061l5.23445,33.15152,38.386-6.97927s-1.74481-33.15152-1.74481-36.64115S371.51453,273.89061,371.51453,273.89061Z" transform="translate(-73.77864 -85)" fill="#a0616a"/><path d="M369.76972,307.04213s43.62042-10.4689,48.85487-6.97927,31.4067,6.97927,31.4067,6.97927l29.66189,45.36524-8.72409,57.57895s-1.74481,92.4753,1.74482,108.17865,22.68262,17.44817-17.44817,20.9378-47.11-1.74481-75.02712,5.23445-50.59969-3.48963-43.62042-10.4689,19.193-118.64754,5.23445-129.11645-19.193-64.55822-19.193-64.55822S369.76972,315.76621,369.76972,307.04213Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><path d="M444.79684,312.27658l5.23445-5.23445,10.4689-10.4689s34.02393,14.83094,32.27912,20.06539-13.08613,35.76875-13.08613,35.76875l-15.70335,5.23445Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/><path d="M398.17835,216.64808l-.32186-2.51594a19.77829,19.77829,0,0,0-6.2856.9877,10.32665,10.32665,0,0,1,.388-4.52107,20.103,20.103,0,0,0-5.81783,2.52934l-1.26884-4.59162-17.4426,8.59632c-6.26123,3.08575-12.86249,6.47669-16.44172,12.46952-3.99031,6.68112-3.29591,15.26752-.56816,22.55582s7.26036,13.73814,10.89358,20.62a51.51652,51.51652,0,0,0,4.95064,8.426c6.58389,8.20751,19.3997,9.86569,28.68263,4.91211,3.42591-1.82815,6.53859-4.56058,7.948-8.17894.84275-2.16359,1.03547-4.51792,1.58763-6.77324,1.14826-4.69012,2.48122-14.37168,6.29067-17.3388,1.67631-1.30565,4.99988,3.02263,6.389,1.41483,3.5699-4.13179,2.82068-10.5038.60359-15.494a113.05265,113.05265,0,0,0-7.4426-14.51667C409.125,223.35867,398.13974,216.3463,398.17835,216.64808Z" transform="translate(-73.77864 -85)" fill="#2f2e41"/><path d="M294.74259,382.06926S252.867,422.2,258.10144,443.13785s47.11005,82.00639,47.11005,82.00639,40.13079,20.9378,45.36524,0-27.91707-15.70335-27.91707-15.70335l-26.17225-62.81341,29.66188-41.87561Z" transform="translate(-73.77864 -85)" fill="#a0616a"/><path d="M343.59746,343.68328l-20.9378-3.48963s-15.70335,17.44817-20.9378,24.42744-15.70335,24.42743-15.70335,24.42743l36.64115,26.17226,27.91707-29.66189Z" transform="translate(-73.77864 -85)" fill="#3f3d56"/></svg>
                                </div>
                            </vs-col>
                        </vs-row>
                    </tab-content>

                    <!-- tab 3 content -->
                    <tab-content title="Au départ ..." class="mb-5 profile-card" icon="fas fa-plane-departure" :before-change="validateStep3">
                        <vs-row vs-justify="space-between">
                            <vs-col vs-align="center" w="7">
                                <vs-row>
                                    <vs-col w="12">
                                        <h3>Dates</h3>
                                        <vs-row vs-type="flex">
                                            <vs-col vs-type="flex" vs-justify="center" vs-align="center" w="6">
                                                <vs-input
                                                        label="Date de départ"
                                                        placeholder="JJ / MM / AAAA"
                                                        v-model="trip.date"
                                                        type="text"
                                                        class="input-date-naiss"
                                                >
                                                    <template v-if="errors.includes('date')" #message-danger>
                                                        La date de voyage est requise ou incorrect
                                                    </template>
                                                </vs-input>
                                            </vs-col>
                                            <vs-col vs-type="flex" vs-justify="flex-end" vs-align="center" w="6">
                                                <vs-input
                                                        type="number"
                                                        v-model="trip.days"
                                                        label="Nombre de jours"
                                                        min="1"
                                                        max="14"
                                                />
                                            </vs-col>
                                        </vs-row>
                                        <h3>Budget</h3>
                                        <div style="padding: 0 5px">
                                            <div class="text__with__input">
                                                <span>Pour ce voyage, prévoir un budget de</span>
                                                <vs-input
                                                        type="number"
                                                        v-model="trip.budget.min"
                                                        min="0"
                                                        max="5000"
                                                        class="input__in__text"
                                                />
                                                <span>€ à</span>
                                                <vs-input
                                                        type="number"
                                                        v-model="trip.budget.max"
                                                        min="0"
                                                        max="5000"
                                                        class="input__in__text"
                                                />
                                                <span>€</span>
                                            </div>
                                        </div>
                                        <h3>Flexibilité</h3>
                                        <div class="list-checkbox justify-start">
                                            <vs-switch v-model="trip.flexibilities" val="dates">Dates</vs-switch>
                                            <vs-switch v-model="trip.flexibilities" val="itinerary">Itinéraire</vs-switch>
                                            <vs-switch v-model="trip.flexibilities" val="budget">Budget</vs-switch>
                                        </div>
                                        <h3>Récapitulatif</h3>
                                        <div style="margin: 10px 0;">
                                            <p>Voyage de <b>{{ trip.days }}</b> jours à partir du <b>{{ trip.date === "" ? "JJ / MM / AAAA" : trip.date }}</b>, prévoir de <b>{{ trip.budget.min }}</b>€ à <b>{{ trip.budget.max }}</b>€.</p>
                                            <div v-if="trip.flexibilities.length > 0">
                                                <p>
                                                    Flexible sur :
                                                </p>
                                                <ul class="list-flext">
                                                    <li v-for="flex in trip.flexibilities">
                                                        <b>
                                                            {{ flex === "dates" ? "Les dates" : "" }}
                                                            {{ flex === "itinerary" ? "L'itinéraire": "" }}
                                                            {{ flex === "budget" ? "Le budget": "" }}
                                                        </b>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </vs-col>
                                </vs-row>
                            </vs-col>
                            <vs-col vs-justify="center" vs-align="center" w="5">
                                <div class="step-form-wizard">
                                    <svg id="a7e92c1a-e25c-441e-bda8-8af90c7203b5" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="996.61127" height="804.00001" viewBox="0 0 996.61127 804.00001"><title>settings</title><path d="M568.9169,56.97925c-50.93658,13.2323-95.86584,48.55976-140.52362,94.90528a76.20054,76.20054,0,0,1-99.83508,115.0871c-52.45648,62.66448-109.734,128.91082-179.6413,183.663-177.9953,139.40814,188.04041,393.65546,420,393.65546S988.917,668.04456,988.917,450.63465,792.44938-1.09,568.9169,56.97925Z" transform="translate(-101.69436 -47.99999)" fill="#f2f2f2"/><ellipse cx="532.61127" cy="773.50001" rx="464" ry="30.5" fill="#3f3d56"/><ellipse cx="545.61127" cy="771.00001" rx="160" ry="17" opacity="0.1"/><path d="M1039.04565,787.50555c0,29.27147-17.399,39.49126-38.86526,39.49126-.49723,0-.99443-.00716-1.48807-.0179-.99444-.02147-1.98173-.06795-2.95469-.13591-19.3736-1.37-34.42248-12.11568-34.42248-39.33745,0-28.16613,35.99283-63.71182,38.70426-66.34815l.00358-.00358c.10375-.10373.1574-.15381.1574-.15381S1039.04565,758.23767,1039.04565,787.50555Z" transform="translate(-101.69436 -47.99999)" v-bind:fill="this.user.color"/><path d="M998.76387,822.51827l14.21541-19.8601-14.2512,22.04212-.03576,2.27862c-.99444-.02147-1.98173-.06795-2.95469-.13591l1.531-29.28221-.01072-.22892.025-.03936.14665-2.76868L983.14265,772.428l14.32987,20.02108.03578.58665,1.1554-22.12442L986.43357,748.0787l12.38036,18.9515,1.20548-45.8728L1000.023,721v.15381l-.20031,36.17169,12.17648-14.34063-12.22657,17.4563-.32192,19.81,11.37161-19.01232-11.41812,21.92765-.17885,11.01392L1015.7265,767.717l-16.56558,30.30525Z" transform="translate(-101.69436 -47.99999)" fill="#3f3d56"/><circle cx="476.79322" cy="192.76422" r="47.76284" fill="#2f2e41"/><polygon points="586.73 614.807 612.259 717.744 637.787 713.626 612.259 595.866 586.73 614.807" fill="#a0616a"/><path d="M723.01135,759.15573s-14.53835,4.91525-12.62191,8.2221,1.91645,23.89429-4.67153,31.30576-23.05792,30.4694-4.11748,31.2929,37.88087-11.529,37.88087-11.529V797.0366l3.294,19.76393h3.294V794.56611s8.235-13.99946,3.294-22.23443-8.16167-15.41149-14.37455-14.70547S723.01135,759.15573,723.01135,759.15573Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><polygon points="486.263 702.097 482.146 724.332 510.968 724.332 510.968 702.097 486.263 702.097" fill="#a0616a"/><path d="M590.4283,767.3907s-8.235-.8235-10.70547,10.70546-24.70491,27.99891-32.93989,30.4694-8.235,10.70546,2.47049,12.35246,37.88088,2.47049,44.46886-6.588a80.72283,80.72283,0,0,1,15.64644-15.64645l-4.11748,23.05793h7.41147s6.588-22.23443,13.176-29.6459-13.176-24.70492-16.46995-24.70492Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><path d="M550.07693,413.28688s-5.76448,15.64645-15.64645,18.11694-83.17322,19.76394-83.17322,19.76394-64.23278,1.647-44.46885,15.64644,51.88033,0,51.88033,0,93.87868-11.529,101.29016-23.88142a166.46131,166.46131,0,0,0,12.35246-26.35191Z" transform="translate(-101.69436 -47.99999)" fill="#a0616a"/><path d="M581.36983,344.11311s-46.93935,76.58525-41.17486,78.23224,59.2918,18.94044,61.76229,12.35246,2.47049-37.05737,2.47049-37.05737Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><path d="M581.36983,344.11311s-46.93935,76.58525-41.17486,78.23224,59.2918,18.94044,61.76229,12.35246,2.47049-37.05737,2.47049-37.05737Z" transform="translate(-101.69436 -47.99999)" opacity="0.1"/><path d="M703.24742,325.99618,809.47856,380.347s60.1153,15.64645,46.93935,27.99891-55.99782-10.70546-55.99782-10.70546L682.66,344.11311Z" transform="translate(-101.69436 -47.99999)" fill="#a0616a"/><path d="M567.37037,283.17432s20.58744,31.29289,18.94044,38.70437c0,0,1.647,4.941-.8235,8.235s-1.647,23.05792-1.647,23.05792l8.235,11.529,10.70547-35.41039,4.941-37.88087s-11.11721-15.2347-9.47022-22.64617S567.37037,283.17432,567.37037,283.17432Z" transform="translate(-101.69436 -47.99999)" fill="#a0616a"/><path d="M632.42666,461.04972s-46.11585,123.52459-46.11585,177.87541-17.29344,116.11311-4.11749,121.87759,79.87924,6.588,81.52623,0-8.235-274.22458-13.176-284.93S632.42666,461.04972,632.42666,461.04972Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><path d="M632.42666,461.04972s-46.11585,123.52459-46.11585,177.87541-17.29344,116.11311-4.11749,121.87759,79.87924,6.588,81.52623,0-8.235-274.22458-13.176-284.93S632.42666,461.04972,632.42666,461.04972Z" transform="translate(-101.69436 -47.99999)" opacity="0.1"/><circle cx="476.38148" cy="209.64591" r="30.4694" fill="#a0616a"/><path d="M587.13431,349.8776l-2.20524-18.58441s-6.85323,9.52594-6.85323,16.93741-4.11749,33.76339,13.176,50.23333,20.58744,19.76394,21.41093,23.05793,4.941,3.294,4.941,3.294L641.48513,521.165s7.41147,256.10765,18.94043,259.40163,46.93935,0,49.40984-4.11748-4.11749-113.64262-4.11749-113.64262,18.11694,97.17267,24.70492,97.17267,10.70546-13.99945,9.05847-14.823-11.529-64.23278-15.64645-83.99672-2.47049-186.93387-2.47049-186.93387,13.17595-21.41093-32.11639-63.40929c0,0-28.82241-83.99672-30.4694-83.99672s-33.76339-45.29235-33.76339-45.29235-18.11694,0-21.41093,10.70547S587.13431,349.8776,587.13431,349.8776Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><path d="M615.13321,288.1153l9.882-6.588s41.17486,24.70492,69.99727,26.35192,27.9989,18.11694,16.46994,29.6459S654.66108,388.582,653.01409,378.7,615.13321,288.1153,615.13321,288.1153Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><ellipse cx="476.79322" cy="179.58826" rx="32.93989" ry="19.76393" fill="#2f2e41"/><circle cx="480.08721" cy="145.00138" r="19.76393" fill="#2f2e41"/><path d="M555.42966,182.29591a19.766,19.766,0,0,1,17.7052-19.65708,19.98219,19.98219,0,0,0-2.05875-.10686,19.76394,19.76394,0,0,0,0,39.52787,19.98219,19.98219,0,0,0,2.05875-.10686A19.766,19.766,0,0,1,555.42966,182.29591Z" transform="translate(-101.69436 -47.99999)" fill="#2f2e41"/><rect x="430.06591" y="424.58801" width="441.47946" height="257.824" transform="translate(-184.48419 67.60969) rotate(-9.54725)" fill="#3f3d56"/><path d="M785.61317,473.58139l-289.1327,48.097a5.94707,5.94707,0,1,1-1.95176-11.73292l289.13269-48.097a5.94708,5.94708,0,0,1,1.95177,11.73292Z" transform="translate(-101.69436 -47.99999)" v-bind:fill="this.user.color" opacity="0.3"/><circle cx="604.16474" cy="432.81963" r="24.63788" v-bind:fill="this.user.color"/><path d="M796.34787,538.11245l-289.1327,48.097a5.94707,5.94707,0,1,1-1.95176-11.73292l289.13269-48.097a5.94708,5.94708,0,0,1,1.95177,11.73292Z" transform="translate(-101.69436 -47.99999)" v-bind:fill="this.user.color" opacity="0.3"/><circle cx="484.16118" cy="519.09892" r="24.63788" v-bind:fill="this.user.color"/><path d="M807.08257,602.64352l-289.1327,48.097a5.94707,5.94707,0,1,1-1.95176-11.73292l289.13269-48.097a5.94708,5.94708,0,1,1,1.95177,11.73292Z" transform="translate(-101.69436 -47.99999)" v-bind:fill="this.user.color" opacity="0.3"/><circle cx="584.56892" cy="568.71294" r="24.63788" v-bind:fill="this.user.color"/><circle cx="275.36982" cy="160.20126" r="54.31516" fill="#ff6584"/></svg>
                                </div>
                            </vs-col>
                        </vs-row>
                    </tab-content>

                    <!-- tab 4 content -->
                    <tab-content title="Où allons-nous ?" class="mb-5 profile-card" icon="fas fa-plane-arrival" :before-change="validateStep4">
                        <vs-row>
                            <vs-col vs-justify="center" vs-align="center" w="7">
                                <h3>Destination</h3>
                                <vs-row style="margin-top: 10px">
                                    <vs-col vs-justify="center" vs-align="center" w="6">
                                        <!--
                                        <vs-select
                                                :state="primary"
                                                class="select-country"
                                                label="Pays"
                                                placeholder="Pays"
                                                v-model="trip.country"
                                                filter
                                        >
                                            <vs-option :key="index" :label="item.translations.fr ===  null ? item.name : item.translations.fr" :value="item.name" v-for="(item,index) in this.countries">
                                                {{ item.translations.fr ===  null ? item.name : item.translations.fr }}
                                            </vs-option>

                                        </vs-select> -->
                                        <vs-select
                                                filter
                                                class="select-country"
                                                label="Pays"
                                                placeholder="Pays"
                                                v-model="trip.country"
                                        >
                                            <vs-option :key="index" :label="item.name" :value="item.name" v-for="(item,index) in this.countries">
                                                {{ item.name }}
                                            </vs-option>
                                        </vs-select>
                                    </vs-col>
                                    <vs-col vs-justify="center" vs-align="center" w="6" v-if="trip.country">
                                        <vs-input label="Ville" v-model="trip.city"/>
                                    </vs-col>
                                </vs-row>
                                <h3 style="margin-top: 20px">Récapitulatif</h3>
                                <p style="margin-top: 10px">
                                    Notre destination sera : <b>{{ trip.country ? trip.country : "..." }}</b>
                                    <span v-if="trip.country">
                                        <span v-if="trip.city">
                                            dans la ville de <b>{{ trip.city }}</b>
                                        </span>
                                        <span v-else>
                                             dans une ville que nous ne connaissons pas encore
                                        </span>
                                    </span>
                                    <br>
                                    Pendant <b>{{ trip.days }}</b> jours à partir du <b>{{ trip.date === "" ? "JJ / MM / AAAA" : trip.date }}</b> avec <b>{{ trip.nbTravelers }}</b> voyageurs, prévoir de <b>{{ trip.budget.min }}</b>€ à <b>{{ trip.budget.max }}</b>€.
                                </p>
                                <div v-if="trip.flexibilities.length > 0">
                                    <p>
                                        Flexible sur :
                                    </p>
                                    <ul class="list-flext">
                                        <li v-for="flex in trip.flexibilities">
                                            <b>
                                                {{ flex === "dates" ? "Les dates" : "" }}
                                                {{ flex === "itinerary" ? "L'itinéraire": "" }}
                                                {{ flex === "budget" ? "Le budget": "" }}
                                            </b>
                                        </li>
                                    </ul>
                                </div>
                            </vs-col>
                            <vs-col vs-justify="center" vs-align="center" w="5">
                                <div class="step-form-wizard">
                                    <svg id="fd01fc53-8d1b-486a-a481-e11f41af06c1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="1134.44691" height="709.78382" viewBox="0 0 1134.44691 709.78382"><title>destinations</title><path d="M1151.03821,381.83623c-41.62683-163.68615-185.46063-281.84467-354.2904-286.5839C707.49258,92.74684,613.57116,122.34882,545.18944,225.937c-122.2397,185.175,7.51192,313.13661,97.97132,373.94142A566.02691,566.02691,0,0,1,760.15631,705.0336c61.10237,72.50779,178.64942,162.60891,318.64264,21.43431C1180.26622,624.14438,1178.76579,490.86738,1151.03821,381.83623Z" transform="translate(-32.77655 -95.10809)" fill="#f2f2f2"/><path d="M189.01629,505.9341c-1.12158-1.66114-2.22412-3.36621-3.27734-5.06836l1.70117-1.05274c1.03906,1.68067,2.12695,3.36329,3.2334,5.002Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M179.70868,490.147c-1.855-3.64063-3.57911-7.40137-5.124-11.17822l1.85059-.75782c1.5249,3.72608,3.22558,7.43653,5.05566,11.02784ZM170.405,467.40431c-1.227-3.89355-2.30566-7.88769-3.20654-11.8706l1.95117-.44141c.88819,3.92969,1.95215,7.86963,3.1626,11.71045Zm-5.418-23.96582c-.5669-4.03613-.97315-8.15185-1.2085-12.23388l1.99707-.11524c.23145,4.02783.63281,8.08887,1.19189,12.0708Zm.58252-24.48047-1.999-.04687c.09619-4.08789.36182-8.21485.78906-12.26563l1.98926.21C165.92694,410.85255,165.66473,414.92433,165.56952,418.958Zm2.54736-24.10009-1.96485-.37305c.76172-4.01074,1.70069-8.03858,2.791-11.97168l1.92773.53418C169.79461,386.92775,168.86834,390.90138,168.11688,394.85793Zm6.47754-23.35254-1.876-.69239c1.41406-3.83105,3.00733-7.65136,4.73535-11.35449l1.8125.8457C177.56121,363.95754,175.98944,367.72609,174.59442,371.50539Zm273.90429-6.18213c-1.56347-3.70752-3.30322-7.39844-5.17138-10.97071l1.77246-.92675c1.89355,3.62109,3.65723,7.3623,5.2417,11.12011ZM184.84784,349.52882l-1.73633-.99218c2.02539-3.543,4.2251-7.04737,6.53808-10.41651L191.298,339.252C189.01629,342.57619,186.84588,346.03371,184.84784,349.52882Zm252.41992-5.68359c-2.15381-3.40185-4.47657-6.75732-6.90381-9.97266l1.59668-1.20507c2.46,3.25927,4.814,6.66015,6.99658,10.10791ZM198.571,329.54054l-1.55078-1.26367c2.58008-3.16553,5.32226-6.26221,8.15185-9.20508l1.44141,1.38575C203.822,323.36134,201.1159,326.417,198.571,329.54054Zm224.09473-5.042c-2.68262-3.00684-5.52246-5.93848-8.44092-8.71387l1.37842-1.44922c2.95752,2.813,5.83593,5.78418,8.55468,8.83106ZM215.37176,312.064l-1.32324-1.499c3.06006-2.70117,6.27051-5.30908,9.542-7.751l1.19629,1.60254C221.55877,306.82619,218.39129,309.39894,215.37176,312.064Zm189.72754-4.273c-3.13721-2.52832-6.418-4.95752-9.75049-7.21924l1.123-1.65527c3.37793,2.293,6.70264,4.75439,9.88232,7.31689ZM234.79461,297.56838l-1.06054-1.69532c3.46289-2.166,7.05566-4.2124,10.6792-6.08252l.917,1.77735C241.75555,293.41261,238.21112,295.43166,234.79461,297.56838Zm150.24366-3.39063c-3.51368-1.98291-7.14844-3.84082-10.80323-5.522l.83594-1.81739c3.70459,1.70459,7.38867,3.5879,10.9502,5.59717ZM256.32342,286.458l-.76758-1.84668c3.769-1.56543,7.64844-2.9917,11.53077-4.23877l.61132,1.9043C263.86834,283.50685,260.04119,284.91359,256.32342,286.458Zm106.68848-2.41259c-3.78516-1.374-7.67676-2.60645-11.56641-3.6626l.52442-1.92969c3.94238,1.07031,7.8872,2.31934,11.72461,3.7124Zm-83.62354-4.99463-.45117-1.94825c3.97168-.92089,8.03613-1.687,12.07959-2.27783l.28906,1.979C287.31659,277.38673,283.30731,278.14259,279.38836,279.0508Zm60.22705-1.3584c-3.95312-.72852-7.99365-1.30176-12.00927-1.70313l.19922-1.99023c4.06982.40723,8.16552.98779,12.17236,1.72656Zm-36.24755-2.146-.126-1.99609c4.06787-.25684,8.21533-.34815,12.291-.27442l-.03613,2C311.47186,275.20216,307.3825,275.29347,303.36786,275.5464Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M454.52215,382.53224c-.53564-1.90332-1.11767-3.82178-1.73047-5.70215l1.90137-.62011c.62109,1.90625,1.21143,3.85107,1.75391,5.78027Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M442.32684,270.53127h-2c0-2.00147.03516-4.03223.105-6.03516l1.999.06934C442.362,266.54591,442.32684,268.55275,442.32684,270.53127Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M443.2785,252.53908l-1.98926-.21192c.42871-4.02783,1.0083-8.08837,1.72315-12.06884l1.96875.35351C444.27459,244.54543,443.70184,248.55861,443.2785,252.53908Zm4.25781-23.70117-1.9375-.49414c1.001-3.9253,2.15821-7.86084,3.43848-11.69776l1.89746.63281C449.66912,221.07082,448.52606,224.95949,447.53631,228.83791ZM766.4992,207.73732c-1.49511-3.71143-3.14453-7.41455-4.90234-11.00537l1.79687-.87891c1.77832,3.63379,3.44727,7.38086,4.96094,11.13721Zm-311.3374-1.75293-1.84668-.76855c1.55518-3.73731,3.26758-7.46827,5.09082-11.08936l1.78613.89941C458.3908,198.60451,456.69842,202.291,455.1618,205.98439ZM755.91522,186.106c-2.01074-3.45605-4.17285-6.88525-6.42774-10.19189l1.65235-1.127c2.28222,3.34619,4.46972,6.81591,6.5039,10.313Zm-289.918-1.647L464.28,183.43361c2.07276-3.47266,4.30176-6.91895,6.624-10.24268l1.63965,1.14551C470.24823,177.62111,468.04559,181.02687,465.99725,184.459Zm276.34961-18.25195c-2.48731-3.13916-5.11621-6.22559-7.81445-9.17383l1.47461-1.35059c2.73046,2.9834,5.3916,6.106,7.9082,9.28223ZM479.79608,164.709l-1.55371-1.25977c2.55127-3.14648,5.24707-6.23925,8.01318-9.19238l1.46,1.36719C484.98162,158.543,482.31707,161.59963,479.79608,164.709Zm246.28808-16.27c-2.90722-2.75293-5.94824-5.43457-9.04-7.97119l1.26758-1.5459c3.12891,2.56592,6.20606,5.27979,9.14746,8.06494Zm-229.82324-1.31152-1.35937-1.46778c2.96972-2.74951,6.07666-5.42773,9.23535-7.95947l1.251,1.56055C502.26629,141.7632,499.19549,144.40968,496.26092,147.12746Zm211.19824-13.96631c-3.26757-2.31152-6.65918-4.5332-10.082-6.60352l1.03516-1.71093c3.46289,2.09472,6.89551,4.34277,10.20117,6.68164Zm-192.40869-1.0957-1.13672-1.64551c3.32959-2.29883,6.78516-4.50635,10.27149-6.562l1.01562,1.72265C521.75555,127.61232,518.34051,129.794,515.05047,132.06545Zm171.79737-11.37159c-3.56543-1.82129-7.24024-3.53418-10.92383-5.09228l.77929-1.8418c3.72754,1.57617,7.44629,3.31006,11.05469,5.15283Zm-151.05957-.853-.88965-1.791c3.623-1.79883,7.35742-3.48877,11.10009-5.02246l.75879,1.85059C543.05877,116.39357,539.36834,118.06349,535.78827,119.84084Zm128.873-8.53077c-3.7832-1.28808-7.667-2.45507-11.543-3.46826l.50586-1.93457c3.92285,1.0249,7.85352,2.20606,11.68164,3.50928Zm-106.60937-.59179-.623-1.90039c3.84375-1.25977,7.78516-2.396,11.71533-3.377l.48438,1.94043C565.74481,108.3506,561.84979,109.47316,558.05194,110.71828Zm83.30566-5.501c-3.93262-.73-7.94238-1.32617-11.918-1.77246l.22266-1.9873c4.02344.45166,8.08105,1.05468,12.06055,1.793Zm-59.94189-.3291-.34278-1.9707c3.98438-.69385,8.04737-1.252,12.07764-1.65967l.20117,1.99023C589.36932,103.65041,585.35321,104.20216,581.41571,104.8882Zm36.00927-2.36035c-4.00488-.1626-8.07617-.18408-12.05078-.06543l-.06054-1.999c4.02246-.12011,8.14062-.09912,12.19336.06641Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M772.34979,224.89894c-.53516-1.90332-1.11328-3.82471-1.7168-5.71191l1.9043-.60938c.61133,1.90918,1.19629,3.85352,1.73828,5.7793Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><ellipse cx="155.07017" cy="433.15038" rx="33.31243" ry="2.49843" v-bind:fill="this.user.color"/><ellipse cx="420.73684" cy="312.3928" rx="33.31243" ry="2.49843" v-bind:fill="this.user.color"/><ellipse cx="748.86432" cy="186.63836" rx="33.31243" ry="2.49843" v-bind:fill="this.user.color"/><rect x="648.23265" y="323.3363" width="141.66201" height="1.99979" transform="translate(-51.508 508.34257) rotate(-45)" fill="#3f3d56"/><rect x="438.22607" y="291.40511" width="168.22803" height="2" fill="#3f3d56"/><rect x="327.42698" y="446.90051" width="133.06486" height="1.99979" transform="translate(-234.10192 314.65027) rotate(-45)" fill="#3f3d56"/><rect x="172.55908" y="412.16292" width="112.42969" height="2" fill="#3f3d56"/><rect x="40.6194" y="575.49866" width="157.99935" height="1.99979" transform="translate(-405.38696 158.32789) rotate(-45)" fill="#3f3d56"/><path d="M187.84686,526.76027A18.48926,18.48926,0,1,1,206.33563,508.271,18.51013,18.51013,0,0,1,187.84686,526.76027Zm0-34.97852A16.48926,16.48926,0,1,0,204.33563,508.271,16.50815,16.50815,0,0,0,187.84686,491.78175Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M335.25409,526.76027A18.48926,18.48926,0,1,1,353.74334,508.271,18.50971,18.50971,0,0,1,335.25409,526.76027Zm0-34.97852A16.48926,16.48926,0,1,0,351.74334,508.271,16.508,16.508,0,0,0,335.25409,491.78175Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M453.51336,406.00246a18.489,18.489,0,1,1,18.48926-18.48926A18.51,18.51,0,0,1,453.51336,406.00246Zm0-34.978a16.489,16.489,0,1,0,16.48926,16.48877A16.50763,16.50763,0,0,0,453.51336,371.02443Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M656.71893,406.00246a18.489,18.489,0,1,1,18.48926-18.48926A18.50992,18.50992,0,0,1,656.71893,406.00246Zm0-34.978a16.489,16.489,0,1,0,16.48926,16.48877A16.5072,16.5072,0,0,0,656.71893,371.02443Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M781.6408,280.24806a18.489,18.489,0,1,1,18.48926-18.48926A18.51012,18.51012,0,0,1,781.6408,280.24806Zm0-34.978a16.489,16.489,0,1,0,16.48926,16.48876A16.508,16.508,0,0,0,781.6408,245.27Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M51.2658,663.34132a18.48926,18.48926,0,1,1,18.48877-18.48925A18.51012,18.51012,0,0,1,51.2658,663.34132Zm0-34.97851a16.48926,16.48926,0,1,0,16.48877,16.48926A16.50814,16.50814,0,0,0,51.2658,628.36281Z" transform="translate(-32.77655 -95.10809)" fill="#3f3d56"/><path d="M187.84672,399.17278a33.31242,33.31242,0,0,0-33.31243,33.31244c0,18.39793,33.31243,79.117,33.31243,79.117s33.31244-60.7191,33.31244-79.117A33.31243,33.31243,0,0,0,187.84672,399.17278Zm0,50.80146a17.489,17.489,0,1,1,17.489-17.489A17.489,17.489,0,0,1,187.84672,449.97424Z" transform="translate(-32.77655 -95.10809)" v-bind:fill="this.user.color"/><path d="M453.51339,274.25115A33.31243,33.31243,0,0,0,420.201,307.56359c0,18.39793,33.31244,79.117,33.31244,79.117s33.31243-60.7191,33.31243-79.117A33.31242,33.31242,0,0,0,453.51339,274.25115Zm0,50.80146a17.489,17.489,0,1,1,17.489-17.489A17.489,17.489,0,0,1,453.51339,325.05261Z" transform="translate(-32.77655 -95.10809)" v-bind:fill="this.user.color"/><rect x="621.17676" y="707.34063" width="453.53514" height="2.44319" fill="#2f2e41"/><path d="M1028.20946,167.53991c-7.05081,20.87569-17.86231,39.89658-39.89659,39.89658s-38.65507-17.89732-39.89658-39.89658c-1.5864-28.11069,17.354-40.013,39.89658-39.89659C1014.94326,127.78087,1036.57709,142.76553,1028.20946,167.53991Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><rect x="915.63974" y="69.28209" width="78.74327" height="99.74147" fill="#2f2e41"/><path d="M1029.47172,238.698h6.69761s16.37193,20.837,8.93014,49.11579-1.48835,33.488-1.48835,33.488,9.68812,30.62354-.73038,76.76262,9.66052,58.67788,9.66052,58.67788l-18.60446-37.20893-10.4185-98.23157Z" transform="translate(-32.77655 -95.10809)" fill="#575a89"/><path d="M1029.47172,241.146h6.69761s16.37193,20.837,8.93014,49.11579-1.48835,33.488-1.48835,33.488,9.68812,30.62354-.73038,76.76262,9.66052,58.67788,9.66052,58.67788l-18.60446-37.20893-10.4185-98.23157Z" transform="translate(-32.77655 -95.10809)" opacity="0.1"/><path d="M1006.0301,202.60535s-5.95343,23.81371-2.97672,23.81371,13.39522,12.651,13.39522,12.651l6.69761,24.55789-5.20925,17.86029-17.86029-3.72089-22.32536-24.5579-6.69761-6.6976-5.95343-21.58118s7.44179-8.93015,2.23254-16.37193S1006.0301,202.60535,1006.0301,202.60535Z" transform="translate(-32.77655 -95.10809)" fill="#fbbebe"/><path d="M1006.0301,202.60535s-5.95343,23.81371-2.97672,23.81371,13.39522,12.651,13.39522,12.651l6.69761,24.55789-5.20925,17.86029-17.86029-3.72089-22.32536-24.5579-6.69761-6.6976-5.95343-21.58118s7.44179-8.93015,2.23254-16.37193S1006.0301,202.60535,1006.0301,202.60535Z" transform="translate(-32.77655 -95.10809)" opacity="0.1"/><path d="M825.93888,268.83724,809.567,242.04681s-17.67265-39.10242-30.32369-24.963,19.161,42.82332,19.161,42.82332l16.37193,21.58118Z" transform="translate(-32.77655 -95.10809)" fill="#fbbebe"/><path d="M1043.239,501.76515,1031.33217,588.834s1.48836,6.69761-13.39521,31.25551-55.06922,78.13875-55.06922,78.13875l-11.90686-22.32536s14.13939-70.697,38.69729-85.58054l-2.97672-107.16172Z" transform="translate(-32.77655 -95.10809)" fill="#fbbebe"/><path d="M1043.239,501.76515,1031.33217,588.834s1.48836,6.69761-13.39521,31.25551-55.06922,78.13875-55.06922,78.13875l-11.90686-22.32536s14.13939-70.697,38.69729-85.58054l-2.97672-107.16172Z" transform="translate(-32.77655 -95.10809)" opacity="0.1"/><path d="M950.96088,666.9728l16.37193,31.2555s-13.39521,37.95311-8.186,41.674,11.90685,37.20893-2.23254,33.488-25.30207-18.60447-25.30207-20.837,8.186-33.488,3.72089-40.92983-7.44179-9.67432-7.44179-9.67432l-11.16267,18.60447-3.7209-2.23254,18.60447-31.2555s2.23253-5.95343,11.16268-9.67432Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><path d="M950.96088,666.9728l16.37193,31.2555s-13.39521,37.95311-8.186,41.674,11.90685,37.20893-2.23254,33.488-25.30207-18.60447-25.30207-20.837,8.186-33.488,3.72089-40.92983-7.44179-9.67432-7.44179-9.67432l-11.16267,18.60447-3.7209-2.23254,18.60447-31.2555s2.23253-5.95343,11.16268-9.67432Z" transform="translate(-32.77655 -95.10809)" opacity="0.1"/><path d="M986.8551,489.41187s5.77978,82.30607-6.87125,103.88725V700.46084l-17.11611,2.23253s-34.56547-56.734-21.91444-112.54736-13.062-106.98533-13.062-106.98533Z" transform="translate(-32.77655 -95.10809)" fill="#fbbebe"/><circle cx="949.81193" cy="94.47413" r="31.2555" fill="#fbbebe"/><path d="M1014.21606,271.814l-31.2555-39.44147-17.86029-11.90686L957.65849,230.14l-20.837,5.95343s-13.39522,2.97671-27.53461,19.34864-39.44147,26.04625-39.44147,26.04625l-20.837-3.72089s-24.55789-11.16268-24.55789-14.1394-21.58118,20.09283-16.37193,29.023,16.37193,20.09282,22.32536,26.04625,8.186,23.06954,44.65071,17.86029,49.86-23.06954,49.86-23.06954,6.69761,34.23222,26.04625,40.92982,82.60383,11.16268,82.60383,11.16268l13.39521-44.65071s14.88357-19.34865-5.20925-47.62743c0,0-.38574-15.81551-2.54584-26.33966-.84019-4.09344-1.94881-7.38636-3.40759-8.63674-5.20925-4.46507-32.74386-11.90686-32.74386-11.90686s4.46508,14.88358,6.69761,15.62775S1017.19278,268.09307,1014.21606,271.814Z" transform="translate(-32.77655 -95.10809)" fill="#575a89"/><path d="M914.124,493.95127c37.47673-2.60462,59.2961,3.53485,69.03,8.21572a24.67563,24.67563,0,0,1,5.07534,3.08091c.71439.60278,1.05668,1.07162,1.05668,1.35441,0,2.23253,51.34832,6.69761,69.95279,2.23253s-12.651-100.46411-20.837-108.65007-3.773-38.1466-3.773-38.1466-16.31978-3.5274-20.78486-2.039-27.5346-2.23253-35.72057-6.6976-18.60446-1.48836-25.30207-2.97672c-2.99161-.66233-4.50232.45394-5.254,1.89765a12.86762,12.86762,0,0,1-5.09006,5.14225c-15.7989,9.10134-28.47973,72.02908-38.0276,106.81943C894.03122,502.13724,914.124,493.95127,914.124,493.95127Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><path d="M982.96056,693.76323l2.97672,16.37193S1001.565,732.46052,997.1,751.065c0,0,3.7209,24.55789,17.11611,31.2555s2.23254,22.32536-10.4185,21.58118-24.55789.74418-26.04625-2.97671-10.4185-40.18565-10.4185-40.18565l-2.44319,42.67115-7.44178.74418,2.44318-49.36876s-5.95342-11.16268,2.23254-29.76714l-2.066-28.5575S980.728,701.205,982.96056,693.76323Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><path d="M983.15406,502.167a24.67563,24.67563,0,0,1,5.07534,3.08091c.39426-7.0325,2.67149-49.56232-.05959-48.87765Z" transform="translate(-32.77655 -95.10809)" opacity="0.1"/><path d="M969.64947,146.5417s-.34615-1.92689-13.88381,3.14973S934.1131,192.23132,940.88193,214.23c2.72781,8.86539.50878,15.25736-3.113,19.72968-13.59109,16.78288-8.24084,41.9903,11.55562,50.62064q.83817.3654,1.71061.72242l8.461,5.07663s-10.786-14.19212-5.38828-32.34645a93.07919,93.07919,0,0,0,1.67965-47.65026c-.61926-2.65858-1.3227-5.39906-2.12175-8.19575C946.897,178.496,950.18907,181.232,973.88,157.54105c0,0,13.53766,10.15325,25.38312,15.22988s21.99871,10.15325,13.53767,27.07533-28.76754,32.152-15.22988,54.15066,15.22988,13.53767,13.53767,21.99871h0l-1.69221,8.461,10.15325-10.15325-6.76883,13.53767s21.9987,0,28.76754-16.92209l3.38441,3.38442s13.53767-23.69091-11.84545-45.68962c0,0-8.461-5.07663,3.38441-21.99871s2.748-53.30456-12.48187-60.07339S969.64947,146.5417,969.64947,146.5417Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><circle cx="955.53633" cy="25.18586" r="15.74865" fill="#2f2e41"/><path d="M969.41449,111.89466a15.74931,15.74931,0,0,1,14.17379-15.66912c-.5181-.05146-1.04318-.07953-1.57487-.07953a15.74865,15.74865,0,0,0,0,31.49731c.53169,0,1.05677-.02807,1.57487-.07953A15.74936,15.74936,0,0,1,969.41449,111.89466Z" transform="translate(-32.77655 -95.10809)" fill="#2f2e41"/><path d="M781.64087,154.32639a33.31243,33.31243,0,0,0-33.31244,33.31243c0,18.39793,33.31244,79.117,33.31244,79.117s33.31243-60.7191,33.31243-79.117A33.31242,33.31242,0,0,0,781.64087,154.32639Zm0,50.80146a17.489,17.489,0,1,1,17.489-17.489A17.489,17.489,0,0,1,781.64087,205.12785Z" transform="translate(-32.77655 -95.10809)" v-bind:fill="this.user.color"/><circle cx="480.32023" cy="661.02324" r="18.02529" fill="#f1f1f1"/><circle cx="592.46685" cy="584.77806" r="18.02529" fill="#f1f1f1"/><circle cx="453.77896" cy="513.7842" r="18.02529" fill="#f1f1f1"/></svg>
                                </div>
                            </vs-col>
                        </vs-row>
                    </tab-content>

                    <!-- tab 5 content -->
                    <tab-content title="Sur place" class="mb-5 profile-card" icon="fas fa-sun">
                        <vs-row>
                            <vs-col vs-justify="center" vs-align="center" w="7">
                                <vs-row vs-type="flex" style="justify-content: space-between; align-items: center;">
                                    <vs-col w="6">
                                        <h3>Activités</h3>
                                    </vs-col>
                                    <vs-col vs-type="flex" vs-justify="flex-end" w="6">
                                        <vs-button @click="activePrompt = true" type="relief" class="btn-perso" style="margin-left: auto;">Ajouter une activité</vs-button>
                                    </vs-col>
                                </vs-row>

                                <vs-table
                                        style="margin-top: 10px"
                                        noDataText="Pas d'activités prévues"
                                >
                                    <template #notFound>
                                        Aucune activité
                                    </template>

                                    <template #thead>
                                        <vs-th>
                                            Nom de l'activité
                                        </vs-th>
                                        <vs-th>
                                            Type de l'activité
                                        </vs-th>
                                        <vs-th>
                                            Volonté
                                        </vs-th>
                                        <vs-th>
                                            Action
                                        </vs-th>
                                    </template>

                                    <template #tbody>
                                        <vs-tr :key="indextr" v-for="(tr, indextr) in trip.activities" >
                                            <vs-td :data="tr.name">
                                                {{ tr.name }}
                                            </vs-td>

                                            <vs-td :data="tr.type">
                                                {{ tr.type }}
                                            </vs-td>

                                            <vs-td :data="tr.wish">
                                                {{ tr.wish }} %
                                            </vs-td>

                                            <vs-td :data="tr.name">
                                                <vs-button @click="deleteActivity(indextr)" radius type="relief" color="primary" icon><i class="fad fa-trash"></i></vs-button>
                                            </vs-td>
                                        </vs-tr>
                                    </template>
                                </vs-table>
                                <vs-dialog
                                        :is-valid="validatedActivity"
                                        title="Ajouter une activité"
                                        class="modal-perso"
                                        accept-text="Ajouter"
                                        v-model="activePrompt"
                                >
                                    <template #header>
                                        <h4 class="not-margin">
                                            Ajout d'une <b>activité</b>
                                        </h4>
                                    </template>
                                    <div class="con-content">
                                        <vs-input label="Nom de l'activité" v-model="activity.name" style="margin-bottom: 5px;"/>
                                        <vs-row vs-type="flex" style="margin-top: 30px;">
                                            <vs-col w="7">
                                                <vs-input label="Type de l'activité" v-model="activity.type" style="margin-bottom: 5px;"/>
                                            </vs-col>
                                            <vs-col w="5">
                                                <vs-input
                                                        label="Volonté"
                                                        type="number"
                                                        v-model="activity.wish"
                                                        min="0"
                                                        max="100"
                                                        class="input__in__text"
                                                />
                                            </vs-col>
                                        </vs-row>
                                    </div>
                                    <template #footer>
                                        <div class="con-footer">
                                            <vs-button @click="activePrompt=false" dark transparent>
                                                Fermer
                                            </vs-button>
                                            <vs-button @click="validActivity()" transparent>
                                                Ajouter
                                            </vs-button>
                                        </div>
                                    </template>
                                </vs-dialog>
                            </vs-col>
                            <vs-col vs-justify="center" vs-align="center" w="5">
                                <div class="step-form-wizard">
                                    <svg id="3d2e1720-bd24-4bc0-a54b-69e5d5c1743b" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="806.22" height="730.95" viewBox="0 0 806.22 730.95"><defs><linearGradient id="72a9d418-5bc5-4c80-a690-71e468528603" x1="562.87" y1="813.16" x2="562.87" y2="356.48" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="gray" stop-opacity="0.25"/><stop offset="0.54" stop-color="gray" stop-opacity="0.12"/><stop offset="1" stop-color="gray" stop-opacity="0.1"/></linearGradient><linearGradient id="616051fd-1701-470f-bedf-ccdfdf794555" x1="736.95" y1="813.71" x2="736.95" y2="595.52" xlink:href="#72a9d418-5bc5-4c80-a690-71e468528603"/></defs><title>departing</title><g opacity="0.6"><polygon points="259.93 41.01 281.34 34.97 314.01 37.62 280.36 47.1 259.93 41.01" v-bind:fill="this.user.color"/><polygon points="259.93 41.01 281.34 34.97 314.01 37.62 280.36 47.1 259.93 41.01" opacity="0.2"/><path d="M404.4,140.64,435,132l16.68,4.52,124.39-35.07s18.87-3.34,19.57,3L435,149.48Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><path d="M580.79,116.27,447.22,153.93,435,149.48l161.1-45.42S600.16,110.81,580.79,116.27Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><path d="M580.79,116.27,447.22,153.93,435,149.48l161.1-45.42S600.16,110.81,580.79,116.27Z" transform="translate(-196.89 -84.52)" opacity="0.2"/><polygon points="270.6 78.84 292.01 72.81 357.87 29.2 310.96 42.43 270.6 78.84" v-bind:fill="this.user.color"/></g><path d="M867.59,494.85c-4.59-1.63-9.46-3.12-14.25-2.29-8,1.4-13.64,9-16,16.79s-2.31,16.1-3.83,24.1-5.16,16.29-12.34,20.13-15.66,2.32-23.63,1.4a149.21,149.21,0,0,0-31.47-.29c-8.79.85-19.1,3.82-21.68,12.26-2.11,6.9,2.08,13.92,4.88,20.57,5.23,12.43,5.65,27.14-.34,39.23-6.09,12.3-18.1,21.1-23.16,33.85-1.28,3.24-2.09,6.83-1.22,10.2a18.63,18.63,0,0,0,3.63,6.58c4,5.12,8.72,9.63,12.34,15,7.55,11.2,9.6,25.16,10.48,38.64a281.8,281.8,0,0,1-.22,39.79c9.84-3.24,20.72-.35,30.86-2.44,21.59-4.45,35.07-30,57.06-31.56,10.4-.73,21.59,4.16,30.87-.6,4.52-2.32,7.73-6.65,9.88-11.26a39.82,39.82,0,0,0,2.39-26.7c-1.63-6.19-4.8-12.24-4.18-18.61.53-5.45,3.79-10.26,7.55-14.24,10.69-11.35,26.21-18.29,33.89-31.86,7.2-12.72,5.38-29.93-4.31-40.86-2.54-2.87-5.56-5.33-7.78-8.45-3.69-5.19-4.87-11.75-5.31-18.11-1.05-15.25,1.54-31.38-3.39-46.19C893.84,506.46,880.19,499.34,867.59,494.85Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color" opacity="0.6"/><path d="M867.33,495.3s-43.77,243.11-100.62,272.1" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M745.54,562.59s46.88,6.44,93.72,54.59" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M831,645.41s36.09-35.29,92.45-29.29" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M724.95,660.75s65.84,5.59,90.22,30.88" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M805.27,715.17s36.89-6.46,70.9,15.3" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M373.08,619c2.86-1,5.9-1.95,8.89-1.43,5,.87,8.51,5.61,10,10.48s1.44,10,2.39,15,3.22,10.17,7.7,12.56,9.77,1.45,14.74.87a93.08,93.08,0,0,1,19.63-.18c5.48.53,11.92,2.38,13.53,7.65,1.32,4.3-1.3,8.69-3,12.84-3.26,7.76-3.52,16.93.21,24.47s11.29,13.16,14.45,21.12a10.44,10.44,0,0,1,.76,6.37,11.62,11.62,0,0,1-2.26,4.1c-2.48,3.19-5.44,6-7.7,9.36-4.71,7-6,15.7-6.54,24.11a175.8,175.8,0,0,0,.14,24.83c-6.14-2-12.92-.22-19.25-1.53C413.22,786.91,404.82,771,391.1,770c-6.49-.46-13.47,2.6-19.26-.37a14.54,14.54,0,0,1-6.16-7,24.84,24.84,0,0,1-1.49-16.65c1-3.86,3-7.64,2.61-11.61-.33-3.4-2.37-6.4-4.71-8.89-6.67-7.08-16.35-11.41-21.14-19.88a22.65,22.65,0,0,1,2.69-25.49,66.9,66.9,0,0,0,4.85-5.27c2.3-3.24,3-7.33,3.31-11.3.66-9.51-1-19.58,2.11-28.82C356.7,626.28,365.22,621.84,373.08,619Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color" opacity="0.6"/><path d="M373.24,619.32S400.55,771,436,789.08" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M449.22,661.3s-29.25,4-58.47,34.05" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M395.89,713s-22.51-22-57.68-18.27" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M462.07,722.54S421,726,405.78,741.8" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M412,756.49s-23-4-44.23,9.55" transform="translate(-196.89 -84.52)" fill="none" stroke="#535461" stroke-miterlimit="10" opacity="0.6"/><path d="M746.87,116.36c-3.38,0-7.13,0-9.7.2a11.8,11.8,0,0,0-3.36.69l-17.35,6.23c14,.35,28.64.24,43,.13L840,123c8.22-.06,16.85-.14,23.71-.63,4.59-.33,8-.81,12.2-1.19,15.25-1.36,37.49-1.1,57.35-1s43.84-.45,50.83-2.44l-42.63,0c-4.6,0-9.42,0-13.48-.23-4.74-.27-7.64-.8-11.81-1.16-6.9-.59-16.42-.66-25.27-.7C842.33,115.46,794.34,116.61,746.87,116.36Z" transform="translate(-196.89 -84.52)" fill="#f4f4f4"/><path d="M501.61,235.88c-1.16,0-2.46,0-3.35.06a4.73,4.73,0,0,0-1.16.2l-6,1.81c4.84.1,9.88.07,14.84,0l27.8-.18c2.84,0,5.81,0,8.18-.18,1.58-.1,2.75-.24,4.21-.35,5.26-.4,12.93-.32,19.78-.3s15.12-.13,17.54-.71H568.75c-1.59,0-3.25,0-4.65-.07-1.64-.08-2.63-.23-4.08-.34-2.38-.17-5.66-.19-8.72-.2C534.54,235.62,518,236,501.61,235.88Z" transform="translate(-196.89 -84.52)" fill="#f4f4f4"/><path d="M861.6,217.18c-2,0-4.25,0-5.79.1a8.17,8.17,0,0,0-2,.35l-10.35,3.13c8.37.18,17.09.12,25.65.07l48.06-.31c4.9,0,10.05-.07,14.14-.32,2.74-.16,4.76-.41,7.28-.6,9.1-.68,22.36-.55,34.21-.52s26.15-.23,30.32-1.23H977.68c-2.74,0-5.62,0-8-.12-2.83-.14-4.55-.4-7-.58-4.11-.3-9.79-.33-15.07-.35C918.54,216.73,889.91,217.31,861.6,217.18Z" transform="translate(-196.89 -84.52)" fill="#f4f4f4"/><path d="M646.34,493.61l.12,0-6.2-33.07a17.91,17.91,0,0,0-5.61-13l-28.67-14-.17.39a32.47,32.47,0,0,1-5.11-8.57l.5-.71a17.18,17.18,0,0,0,3.3-6.72,11.38,11.38,0,0,0,.17-1.9l.43-.22a10.62,10.62,0,0,0,11.82-8.6c0-.24.08-.49.11-.73a10.63,10.63,0,0,1-9.66,8,29.2,29.2,0,0,0,13-24.29c0-.41,0-.81,0-1.21a10.59,10.59,0,0,0,4.88-7.11c0-.24.08-.49.11-.73a10.6,10.6,0,0,1-5.07,6.6,29.06,29.06,0,0,0-1.79-7.91c.24,0,.48,0,.72,0a10.63,10.63,0,0,0,10.63-10.63c0-.25,0-.49,0-.74a10.55,10.55,0,0,1-12,9.79l-.23-.52a17.39,17.39,0,0,0,12.22-14.15c0-.25,0-.49,0-.74a10.57,10.57,0,0,1-2.14,5.69,10.62,10.62,0,0,0-21-3.15q-.93-.58-1.9-1.1a10.63,10.63,0,0,1,9.61-7.77l-.74,0a10.63,10.63,0,0,0-10.16,7.17,29.22,29.22,0,0,0-36.8,10.54c-2.25,1.63-5.15,4-5,6.07a20.21,20.21,0,0,0-2.53,3.11c-4,6.37-3.33,14.47-3.31,22s-1.34,16.1-7.62,20.26a10.07,10.07,0,0,0,7.79.06,29.87,29.87,0,0,0,6.89-4c1.85-1.32,3.76-2.7,4.86-4.69.08-.15.16-.3.23-.45a24.14,24.14,0,0,0,4.39,2.32c.61,4.94.74,10.64-1.45,12.52l-.33.28-.09.07a9.38,9.38,0,0,1,.42-.95s-5.91,5.61-18.6,5.31-19.19,12.4-19.19,12.4l-8.86,38.09.14,0-.14.62.14,0-.14.62.24-.05a2.86,2.86,0,0,0,2.13,2.1c.77.18,1.72.43,2.79.76,0,7.18-.33,16.58-1.91,19,0,0-4.13,21.85-3.25,28,.67,4.7,1.85,29.23,2.38,40.84a121,121,0,0,0-2.38,12l-13.29,79.72S501,734,501.54,753.28l-1.74,11.13s-27,33.87-19.05,38.64,20.11,6.88,31.76-11.11a29.14,29.14,0,0,1,7.41-11.11,10.92,10.92,0,0,0,3.67-9.3l-.79-7.89,1.42.48.08-.27.8.27s13.29-43.7,11.81-74.11c0,0,24.65-87.67,43.32-101.46l0-.58a9.22,9.22,0,0,1,1.73-.86l4,84.89-4,93.57L581,771.2s-27,33.87-19.05,38.64,20.11,6.88,31.76-11.11a29.14,29.14,0,0,1,7.41-11.11,10.92,10.92,0,0,0,3.67-9.3l-1.24-12.38c4.59-15.73,16.67-61.15,11.61-87.45l8.2-55.92c8.27-9.29,7.16-32.95,7.16-32.95,6.5-13.58,8-67,8-67l1.64-26.37a22,22,0,0,1,3.68-.29,2.46,2.46,0,0,0,.81-.14l.52.07.23-.48a2.47,2.47,0,0,0,.61-.7l.49.15-.12-.63.12,0Zm-23.1,53.62c-.74-1.83-1.56-3.65-2.47-5.45l-.66.07a5.51,5.51,0,0,0-.22-.66,3.21,3.21,0,0,1-.18-.62,3.45,3.45,0,0,1,.21-.66,22.78,22.78,0,0,0,1-2.72,10.31,10.31,0,0,0,1.57-5.68C623.47,535.37,623.6,541,623.23,547.23Zm-2.47-22.58L619.08,507q.25-.72.52-1.39L621.82,528ZM542.52,527c-1.21,1.09-1.53,3-1.48,4.8,0,.2,0,.39,0,.59a20.05,20.05,0,0,0,.74,5.3l-2.59,11,.38-21.12a46.72,46.72,0,0,0,4.42-7.12C543.83,523.77,543.38,526.24,542.52,527Z" transform="translate(-196.89 -84.52)" fill="url(#72a9d418-5bc5-4c80-a690-71e468528603)"/><path d="M559.23,390.73c-3.86,6.14-3.21,13.94-3.19,21.2s-1.29,15.51-7.34,19.52a9.7,9.7,0,0,0,7.5.05,28.78,28.78,0,0,0,6.63-3.88,14.77,14.77,0,0,0,4.68-4.52c1.48-2.68,1.15-5.95,1.08-9A95.37,95.37,0,0,1,570,398c.48-3.27,3.65-12.26-.25-14C566.23,382.45,560.8,388.22,559.23,390.73Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M504.66,741.52l-2.55,16.32s-26,32.63-18.36,37.22,19.38,6.63,30.59-10.71a28.08,28.08,0,0,1,7.14-10.71h0a10.52,10.52,0,0,0,3.54-9l-2.52-25.2Z" transform="translate(-196.89 -84.52)" fill="#333"/><path d="M582.89,748.06l-2.55,16.32S554.33,797,562,801.6s19.38,6.63,30.59-10.71a28.08,28.08,0,0,1,7.14-10.71h0a10.52,10.52,0,0,0,3.54-9L600.73,746Z" transform="translate(-196.89 -84.52)" fill="#333"/><path d="M542.41,550.74s-12.23,8.82-16.78,39.54l-12.8,76.8S502,736.78,504,750.15l21.62,7.4s12.8-42.1,11.38-71.4c0,0,26-92.59,44.23-99.13l3.84,81.78-4,92.16,18.49,3.7s18.77-58.6,12.8-89.6l9.67-66s11.09-35.27-4.27-65.71Z" transform="translate(-196.89 -84.52)" fill="#4d8af0"/><g opacity="0.1"><path d="M579.6,588.4l-.07-1.39c-18.21,6.54-44.23,99.13-44.23,99.13,1.31,26.94-9.41,64.69-11.14,70.6l2.32.8s12.8-42.1,11.38-71.4C537.86,686.15,561.61,601.68,579.6,588.4Z" transform="translate(-196.89 -84.52)"/><path d="M618.65,543.35l-2.44.24c15.18,30.38,4.15,65.47,4.15,65.47l-9.67,66c5.66,29.36-10.88,83.48-12.65,89.12l2.41.48s18.77-58.6,12.8-89.6l9.67-66S634,573.79,618.65,543.35Z" transform="translate(-196.89 -84.52)"/></g><path d="M525.77,490.58a2.76,2.76,0,0,0,2.14,2.68,42.77,42.77,0,0,1,16.35,7.84c9.1,7.11-9.67-14.22-9.67-14.22l-6.26,1.42-2.56,2.28Z" transform="translate(-196.89 -84.52)" fill="#f55f44"/><path d="M525.77,490.58a2.76,2.76,0,0,0,2.14,2.68,42.77,42.77,0,0,1,16.35,7.84c9.1,7.11-9.67-14.22-9.67-14.22l-6.26,1.42-2.56,2.28Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M615.09,509.36s14.75-10.33,25.74-10.18a2.39,2.39,0,0,0,2.42-2.39h0S621.64,488.31,615.09,509.36Z" transform="translate(-196.89 -84.52)" fill="#f55f44"/><path d="M643.25,496.79s-21.62-8.49-28.16,12.56c0,0,15.93-12.23,27-10.24" transform="translate(-196.89 -84.52)" opacity="0.1"/><circle cx="393.17" cy="312.75" r="28.16" fill="#72351c"/><path d="M570.58,417s4,16.5,0,19.91-11.95,9.67-6,14.51,27.59,13.09,34.7,9.1,15.36-2.84,13.37-11.09S595,442.93,594.75,402Z" transform="translate(-196.89 -84.52)" fill="#fdb797"/><path d="M530.75,485.6s.85,22.76-1.71,26.74c0,0-4,21-3.13,27s2.56,45.23,2.56,45.23-3.13,34.42,13.65,31.29-2.56-39.26-2.56-39.26l.85-46.94s7.68-9.67,8.53-21S530.75,485.6,530.75,485.6Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M530.47,485.6s.85,22.76-1.71,26.74c0,0-4,21-3.13,27s2.56,45.23,2.56,45.23-3.13,34.42,13.65,31.29-2.56-39.26-2.56-39.26l.85-46.94s7.68-9.67,8.53-21S530.47,485.6,530.47,485.6Z" transform="translate(-196.89 -84.52)" fill="#fdb797"/><path d="M637.71,488.16l-2.28,36.7s-1.42,51.49-7.68,64.57c0,0,1.71,36.13-15.08,35.27s2.84-39.54,2.84-39.54,9-40.16,4.36-52.93l-4.36-44.07Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M638,488.16l-2.28,36.7s-1.42,51.49-7.68,64.57c0,0,1.71,36.13-15.08,35.27s2.84-39.54,2.84-39.54,9-40.16,4.36-52.93l-4.36-44.07Z" transform="translate(-196.89 -84.52)" fill="#fdb797"/><path d="M583.38,452.61s13.59,6.25,21-12.38L632,453.74a17.26,17.26,0,0,1,5.4,12.52l6,31.86s-20.2-6.83-26.45,11.95L618.65,528l.06.18a19.31,19.31,0,0,1-.89,13.38,3,3,0,0,0,0,2.37c1.42,3.41-.28,10.52-8,11.38s-16.21,0-22.19,1.71-23.89-3.41-29.87,0-18.49-2.56-18.49-2.56l3.41-14.51s-2.28-7.11.57-9.67,1.14-24.75-1.42-31c0,0-10.24-9.67-16.21-8l8.53-36.7s6.26-12.23,18.49-11.95,17.92-5.12,17.92-5.12S563.46,450.62,583.38,452.61Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M584.8,452c3.68,1,13.62,2.1,19.59-12.87L632,452.61a17.26,17.26,0,0,1,5.4,12.52l6,31.86s-20.2-6.83-26.45,11.95l1.71,17.92,1.12,3.56a10.15,10.15,0,0,1-1.42,9.09,3.22,3.22,0,0,0-.55,3.28c1.42,3.41-.28,10.52-8,11.38s-16.21,0-22.19,1.71-23.89-3.41-29.87,0-18.49-2.56-18.49-2.56l3.41-14.51s-2.28-7.11.57-9.67,1.14-24.75-1.42-31c0,0-10.24-9.67-16.21-8l8.53-36.7s6.26-12.23,18.49-11.95,17.92-5.12,17.92-5.12-6.65,12.24,10.92,14.84A28.22,28.22,0,0,1,584.8,452Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M584.8,452.52c3.68,1,13.62,2.1,19.59-12.87L632,453.18a17.26,17.26,0,0,1,5.4,12.52l6,31.86s-20.2-6.83-26.45,11.95l1.71,17.92.06.18a19.31,19.31,0,0,1-.89,13.38,3,3,0,0,0,0,2.37c1.42,3.41-.28,10.52-8,11.38s-16.21,0-22.19,1.71-23.89-3.41-29.87,0-18.49-2.56-18.49-2.56l3.41-14.51s-2.28-7.11.57-9.67,1.14-24.75-1.42-31c0,0-10.24-9.67-16.21-8l8.53-36.7s6.26-12.23,18.49-11.95S570.58,437,570.58,437s-6.65,12.24,10.92,14.84A28.22,28.22,0,0,1,584.8,452.52Z" transform="translate(-196.89 -84.52)" fill="#f55f44"/><path d="M595.32,403.4l-24.18,15.08a78.59,78.59,0,0,1,1.4,7.85,23.25,23.25,0,0,0,8.7,1.68c6,0,11.51-3.38,15.65-7.11A94,94,0,0,1,595.32,403.4Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><circle cx="383.79" cy="318.73" r="23.33" fill="#fdb797"/><circle cx="418.2" cy="290.57" r="10.24" fill="#72351c"/><path d="M617.08,385.9a10.24,10.24,0,0,1-10.21-9.53c0,.24,0,.47,0,.71a10.24,10.24,0,1,0,20.48,0c0-.24,0-.48,0-.71A10.24,10.24,0,0,1,617.08,385.9Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M611.23,396.07a10.24,10.24,0,0,1-8.32-11.22c-.06.23-.11.46-.16.69a10.24,10.24,0,1,0,20.14,3.71c0-.24.08-.47.1-.7A10.24,10.24,0,0,1,611.23,396.07Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M603.27,420.54a10.24,10.24,0,0,1-8.32-11.22c-.06.23-.11.46-.16.69a10.24,10.24,0,1,0,20.14,3.71c0-.24.08-.47.1-.7A10.24,10.24,0,0,1,603.27,420.54Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M617.08,380.5A10.24,10.24,0,0,1,606.87,371c0,.24,0,.47,0,.71a10.24,10.24,0,0,0,20.48,0c0-.24,0-.48,0-.71A10.24,10.24,0,0,1,617.08,380.5Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M602.79,375a10.24,10.24,0,0,1,9.64-10.11l-.71,0a10.24,10.24,0,1,0-.22,20.48l.71,0A10.24,10.24,0,0,1,602.79,375Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M613.1,471.52c2,5.69,0,15.36,0,15.36s-1.42,13.37.85,15.65" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M544,474.65c2,5.69,0,15.36,0,15.36s-1.42,13.37.85,15.65" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M543.41,529.55s2,1.14,6.83,0,7.4-.28,8.25,0" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M546.82,509.36s13.09.85,23.61-3.7c0,0,29,7.11,37.26,2" transform="translate(-196.89 -84.52)" opacity="0.1"/><rect x="328.88" y="547.43" width="17.92" height="11.09" rx="5.55" ry="5.55" fill="#fdb797"/><path d="M525.77,634.8s9.17-.43,9.59-.43,8.34,1.28,8.34,1.28h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.35,588.72s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.35,590.71s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.35,592.7s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.35,594.69s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,642.67s9.17-.33,9.59-.33,8.34,1,8.34,1h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,641.68s9.17-.33,9.59-.33,8.34,1,8.34,1h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,640.68s9.17-.33,9.59-.33,8.34,1,8.34,1h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,639.68s9.17-.33,9.59-.33,8.34,1,8.34,1h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,633.52s9.17-.43,9.59-.43,8.34,1.28,8.34,1.28h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M525.77,631.81s9.17-.43,9.59-.43,8.34,1.28,8.34,1.28h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M584.67,388.32c-1.9,8.17,1.07,17.19,3.26,25.72s3.39,18.56-1.26,24.76A8.14,8.14,0,0,0,594,437a28.14,28.14,0,0,0,5.27-6.2,16.55,16.55,0,0,0,3.18-6.47c.63-3.52-.66-7.28-1.66-10.86a139.88,139.88,0,0,1-3.44-19.25c-.52-4-.13-15.33-4.44-16.4C589,376.85,585.44,385,584.67,388.32Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M585.24,388c-1.9,8.17,1.07,17.19,3.26,25.72s3.39,18.56-1.26,24.76a8.14,8.14,0,0,0,7.29-1.79,28.14,28.14,0,0,0,5.27-6.2,16.55,16.55,0,0,0,3.18-6.47c.63-3.52-.66-7.28-1.66-10.86a139.88,139.88,0,0,1-3.44-19.25c-.52-4-.13-15.33-4.44-16.4C589.54,376.57,586,384.69,585.24,388Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M575.19,394.15c1.94,2.07,2.39,5.16,4.07,7.45,2.53,3.47,7.26,4.42,11.55,4.6a6.5,6.5,0,0,1,3.36.72c1.81,1.19,1.86,3.81,3.07,5.6,2.06,3.06,7.07,2.65,9.69.05s3.39-6.51,3.71-10.18a63.57,63.57,0,0,0-.13-12.29c-.5-4.62-1.7-9.53-5.21-12.58-2.54-2.2-5.93-3.1-9.19-3.93-4.63-1.18-9.53-2.37-14.12-1a25.46,25.46,0,0,0-6.44,3.25q-4.23,2.69-8.33,5.57c-3.08,2.16-8.79,6.44-3.21,8.76C567.7,391.67,572.22,391,575.19,394.15Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><path d="M575.76,393.87c1.94,2.07,2.39,5.16,4.07,7.45,2.53,3.47,7.26,4.42,11.55,4.6a6.5,6.5,0,0,1,3.36.72c1.81,1.19,1.86,3.81,3.07,5.6,2.06,3.06,7.07,2.65,9.69.05s3.39-6.51,3.71-10.18a63.57,63.57,0,0,0-.13-12.29c-.5-4.62-1.7-9.53-5.21-12.58-2.54-2.2-5.93-3.1-9.19-3.93-4.63-1.18-9.53-2.37-14.12-1a25.46,25.46,0,0,0-6.44,3.25q-4.23,2.69-8.33,5.57c-3.08,2.16-8.79,6.44-3.21,8.76C568.27,391.39,572.79,390.69,575.76,393.87Z" transform="translate(-196.89 -84.52)" fill="#72351c"/><path d="M590.94,588.72s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.94,590.71s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.94,592.7s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M590.94,594.69s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M586.81,704.28s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M586.81,706.27s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M586.81,708.26s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M586.81,710.25s9.17-.28,9.59-.28,8.34.85,8.34.85h-2.92Z" transform="translate(-196.89 -84.52)" fill="#f0e4e4"/><path d="M777.47,666.08H761.52V601.52h5.29v-6H707.09v6H713v64.56H696.44c-11.46,0-20.76,9-20.76,20v98.84a20,20,0,0,0,13.16,18.62,8.67,8.67,0,0,0-.09,1.2,9.31,9.31,0,0,0,18.59.19h60.51a9.32,9.32,0,0,0,18.56-.79,8.68,8.68,0,0,0-.08-1.13,20,20,0,0,0,11.9-18.09V686.08C798.22,675,788.93,666.08,777.47,666.08ZM718,601.52h38.57v64.56H718Z" transform="translate(-196.89 -84.52)" fill="url(#616051fd-1701-470f-bedf-ccdfdf794555)"/><rect x="517.05" y="516.98" width="4.78" height="74.12" fill="#333"/><rect x="558.89" y="516.98" width="4.78" height="74.12" fill="#333"/><rect x="511.37" y="513.39" width="57.39" height="5.98" fill="#333"/><rect x="481.18" y="583.74" width="117.76" height="138.46" rx="36.08" ry="36.08" v-bind:fill="this.user.color"/><circle cx="502.67" cy="722.01" r="8.94" fill="#333"/><circle cx="502.67" cy="722.01" r="3.72" fill="#f0e4e4"/><circle cx="578.65" cy="721.42" r="8.94" fill="#333"/><circle cx="578.65" cy="721.42" r="3.72" fill="#f0e4e4"/><path d="M774.27,668.45H699.64a21.5,21.5,0,0,0-15.2,6.27A21.47,21.47,0,0,1,694.86,672h74.63a21.57,21.57,0,0,1,21.57,21.57v95.32a21.5,21.5,0,0,1-6.37,15.3,21.56,21.56,0,0,0,11.15-18.89V690A21.57,21.57,0,0,0,774.27,668.45Z" transform="translate(-196.89 -84.52)" opacity="0.1"/><circle cx="539.77" cy="652.67" r="20.32" opacity="0.1"/><path d="M334.1,794.52H299.25S288.36,413.39,334.1,295.78h8.71S312.32,727,334.1,794.52Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><path d="M310.14,276.18s-28.31,78.4-15.25,113.25c0,0,19.6-71.87,42.47-82.76s46.83,82.76,46.83,82.76,13.07-58.8-10.89-106.72c0,0,47.91,26.13,71.87,76.23,0,0,4.36-32.67-52.27-100.18,0,0,50.09,13.07,82.76,43.56,0,0,10.89-24-56.63-63.16,0,0,8.71-28.31,61-15.25,0,0,0-37-82.76-30.49,0,0,0-24,54.45-19.6,0,0,0-30.49-63.16-19.6,0,0,13.07-19.6,21.78-24,0,0-17.42-2.18-30.49,8.71,0,0,2.18-39.2-30.49-54.45L345,139s-36.61-43.14-28.31-34.85c2.18,2.18,8.71,28.31,6.53,41.38,0,0-37-30.49-82.76,2.18,0,0,56.63,4.36,65.34,24,0,0-47.91-8.71-58.8-2.18,0,0,37,13.07,34.85,21.78,0,0-65.34,0-74,24,0,0,67.52,8.71,71.87,15.25,0,0-78.4,56.63-82.76,71.87,0,0,80.58-47.91,93.65-47.91s-58.8,69.69-54.45,87.12C236.09,341.52,294.89,271.83,310.14,276.18Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><g opacity="0.5"><path d="M681.71,705.34h26.09S716,420,681.71,331.89h-6.52S698,654.79,681.71,705.34Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><path d="M699.65,317.22s21.2,58.71,11.42,84.8c0,0-14.68-53.82-31.8-62s-35.06,62-35.06,62-9.78-44,8.15-79.91c0,0-35.88,19.57-53.82,57.08,0,0-3.26-24.46,39.14-75,0,0-37.51,9.78-62,32.62,0,0-8.15-17.94,42.4-47.29,0,0-6.52-21.2-45.66-11.42,0,0,0-27.72,62-22.83,0,0,0-17.94-40.77-14.68,0,0,0-22.83,47.29-14.68,0,0-9.78-14.68-16.31-17.94,0,0,13-1.63,22.83,6.52,0,0-1.63-29.35,22.83-40.77l3.26,40.77s27.41-32.3,21.2-26.09c-1.63,1.63-6.52,21.2-4.89,31,0,0,27.72-22.83,62,1.63,0,0-42.4,3.26-48.92,17.94,0,0,35.88-6.52,44-1.63,0,0-27.72,9.78-26.09,16.31,0,0,48.92,0,55.45,17.94,0,0-50.55,6.52-53.82,11.42,0,0,58.71,42.4,62,53.82,0,0-60.34-35.88-70.12-35.88s44,52.18,40.77,65.23C755.1,366.14,711.07,314,699.65,317.22Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/></g><g opacity="0.3"><path d="M821.55,559H806.28s-4.77-166.92,15.26-218.43h3.82S812,529.43,821.55,559Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/><path d="M811.05,332s-12.4,34.34-6.68,49.6c0,0,8.58-31.48,18.6-36.25s20.51,36.25,20.51,36.25,5.72-25.75-4.77-46.74c0,0,21,11.45,31.48,33.38,0,0,1.91-14.31-22.89-43.88,0,0,21.94,5.72,36.25,19.08,0,0,4.77-10.49-24.8-27.66,0,0,3.82-12.4,26.71-6.68,0,0,0-16.22-36.25-13.35,0,0,0-10.49,23.85-8.58,0,0,0-13.35-27.66-8.58,0,0,5.72-8.58,9.54-10.49,0,0-7.63-1-13.35,3.82,0,0,1-17.17-13.35-23.85l-1.91,23.85s-16-18.89-12.4-15.26c1,1,3.82,12.4,2.86,18.12,0,0-16.22-13.35-36.25,1,0,0,24.8,1.91,28.62,10.49,0,0-21-3.82-25.75-1,0,0,16.22,5.72,15.26,9.54,0,0-28.62,0-32.43,10.49,0,0,29.57,3.82,31.48,6.68,0,0-34.34,24.8-36.25,31.48,0,0,35.29-21,41-21S776.71,353,778.62,360.6C778.62,360.6,804.38,330.08,811.05,332Z" transform="translate(-196.89 -84.52)" v-bind:fill="this.user.color"/></g></svg>
                                </div>
                            </vs-col>
                        </vs-row>
                    </tab-content>
            </form-wizard>
        </div>
    </div>
</template>

<script>
    import {FormWizard, TabContent} from 'vue-form-wizard';
    import Cleave from 'cleave.js';
    import axios from 'axios';
    import moment from "moment";

    const feather = require('feather-icons');

    export default {
        name: 'AddTrip',
        props: {
            user: Object
        },
        data: () => ({
            errors: [],
            activePrompt:false,
            trip: {
                title: '',
                nbTravelers: 1,
                budget: {
                    min: 0,
                    max: 5000
                },
                date: '',
                days: 1,
                flexibilities: [],
                country: '',
                city: '',
                activities: [],
                description: ''
            },
            activity: {
                name: '',
                wish: 50,
                type: '',
            },
            countries: JSON
        }),
        mounted() {
            feather.replace();
            this.cleave = new Cleave('.input-date-naiss input', {
                date: true,
                delimiter: ' / ',
                datePattern: ['d', 'm', 'Y']
            });
            axios
                .get('https://restcountries.eu/rest/v2/all')
                .then(response => (this.countries = response.data))
            ;
        },
        components: {
            FormWizard,
            TabContent
        },
        computed: {
            validatedActivity(){
                return (this.activity.name.length > 0 && this.activity.type.length > 0);
            }
        },
        methods: {
            cancelActivity() {
                this.resetActivity();
            },
            validActivity() {
                this.trip.activities.push({
                    name: this.activity.name,
                    type: this.activity.type,
                    wish: this.activity.wish
                });
                this.resetActivity();
                this.activePrompt = false;
            },
            resetActivity() {
                this.activity.name='';
                this.activity.type='';
                this.activity.wish=50;
            },
            deleteActivity(activity) {
                this.trip.activities.splice(activity, 1);
            },
            validateStep1() {
                this.errors = [];

                if(this.trip.title === "") {
                    this.errors.push('title');
                }
                if (this.trip.nbTravelers === "") {
                    this.trip.nbTravelers = 1
                }
                if(this.trip.description === "") {
                    this.errors.push('description');
                }

                return this.errors.length <= 0;
            },
            validateStep3() {
                this.errors = [];

                if(!(moment(this.trip.date, "DD/MM/YYYY").isValid()) || (moment(this.trip.date, "DD/MM/YYYY").diff(moment())) <= 0) {
                    this.errors.push('date');
                }

                return this.errors.length <= 0;
            },
            validateStep4() {
                this.errors = [];

                if(this.trip.country === ""){
                    this.errors.push('country');
                }

                return this.errors.length <= 0;
            },
            addTrip() {
                const loading = this.$vs.loading();
                axios({
                    method: 'post',
                    url: '/api/add-trip',
                    data: this.trip
                })
                .then(response => {
                    loading.close();
                    const noti = this.$vs.notification({title:'Ajout réussi',text:'La proposition de votre voyage a bien été ajoutée',color:'success',icon:'<i class="fad fa-check"></i>'})
                })
                .catch(response => {
                    loading.close();
                    const noti =  this.$vs.notification({title:'Erreur',text:'Une erreur interne est survenue, veuillez réessayer utltérieurement',color:'danger',icon:'<i class="fad fa-times"></i>'})
                })
            }
        }
    }
</script>
